...
class CommandRegistry {
	...
	registerGroup(group, name, guarded) {
		...
		if(existing) {
			...
			this.client.emit('debug', `Группа ${group.id} уже зарегестрированна; переименована в "${group.name}".`);
		} else {
			...
			this.client.emit('debug', `Зарегистрированная группа ${group.id}.`);
		}
	...
	registerGroups(groups) {
		if(!Array.isArray(groups)) throw new TypeError('Группы должны быть массивом.');
