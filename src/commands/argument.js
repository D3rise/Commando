	...
	async obtain(msg, val, promptLimit = Infinity) {
		...
		while(!valid || typeof valid === 'string') {
			...
			prompts.push(await msg.reply(stripIndents`
				...
				${oneLine`
				Напишите \`cancel\` для выхода из команды, или \`finish\` для завершения ввода.
		...
		while(true) { // eslint-disable-line no-constant-condition
			...
			while(!valid || typeof valid === 'string') {
				...
				if(val) {
					...
					prompts.push(await msg.reply(stripIndents`
						${valid ? valid : oneLine`
							Вы указали неверный ${this.label},
							"${escaped.length < 1850 ? escaped : '[слишком длинное для показа]'}".
							Попробуйте еще раз.
						`}
						${oneLine`
							Напишите \`cancel\` для выхода из команды, или \`finish\` для завершения ввода.
							${wait ? `Команда будет автоматически покинута через ${this.wait} секунд, если вы не ответите.` : ''}
						`}
					`));
