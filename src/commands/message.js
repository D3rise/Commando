...
class CommandMessage {
	...
	async run() { // eslint-disable-line complexity
		...
		if(this.command.guildOnly && !this.message.guild) {
			...
			return this.reply(`Команда \`${this.command.name}\` может быть использована только в канале на сервере.`);
		}
		...
		if(this.command.nsfw && !this.message.channel.nsfw &&
			...
			return this.reply(`Команда \`${this.command.name}\` может быть использована только в NSFW каналах.`);
		}
		...
		if(!hasPermission || typeof hasPermission === 'string') {
			...
			else return this.reply(`У вас нету разрешений для запуска команды \`${this.command.name}\``);
		}
		...
		if(this.message.channel.type === 'text' && this.command.clientPermissions) {
			...
			if(missing.length > 0) {
				...
				if(missing.length === 1) {
					return this.reply(
						`Мне нужно разрешение "${permissions[missing[0]]}" для запуска команды \`${this.command.name}\``
					);
				}
				return this.reply(oneLine`
					Мне нужны следующие разрешения для запуска команды \`${this.command.name}\`:
		...
		if(throttle && throttle.usages + 1 > this.command.throttling.usages) {
			...
			return this.reply(
				`Вы не можете использовать \`${this.command.name}\` еще раз в течении слудующих ${remaining.toFixed(1)} секунд.`
			);
		}
		...
		try {
			...
			if(err instanceof FriendlyError) {
				...
				return this.reply(stripIndents`
					Произошла ошибка при выполнении команды: \`${err.name}: ${err.message}\`
